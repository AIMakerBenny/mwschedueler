...
function participantSummary(event){
  ...
  const rows=(event?.participants||[]).map(id=>{
    const person=contacts.get(String(id));
    if(!person)return '';
    const status=event?.participantStatuses?.[id]==='planned'?'예정':'확정';
    return String(person.name||'이름 없음')+' · '+status;
  }).filter(Boolean);
  return rows.length?rows.join(', '):'참가자 없음';
}
function gameName(event){
  return String(event?.steamGame?.name||'').trim()||'연결된 게임 없음';
}
...
function renderSheet(){
  ...
  list.innerHTML=events.map(event=>{
    ...
    return `<button type="button" class="mws-mobile-day-event-v130" data-mws-event-id="${esc(event.id)}" style="--mws-day-event-color:${esc(color)}">
      ...
      ${event.restDay?'':`<span class="mws-mobile-day-event-row-v130"><b>참가자</b><span>${esc(participantSummary(event))}</span></span>
      <span class="mws-mobile-day-event-row-v130"><b>게임</b><span>${esc(gameName(event))}</span></span>
