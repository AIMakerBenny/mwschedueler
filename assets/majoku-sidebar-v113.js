/* Mawang Scheduler v1.1.3 - Majoku Castle game sidebar and records cleanup */
(()=>{
'use strict';
if(window.__mwsMajokuSidebarV113)return;
window.__mwsMajokuSidebarV113=1;
...
function installStyle(){
  if(document.getElementById('mwsMajokuSidebarStyleV113'))return;
  const style=document.createElement('style');
  style.id='mwsMajokuSidebarStyleV113';
  style.textContent=`
...
.sidebar .nav.mws-majoku-game-nav button[data-castle-game-target]{display:grid;grid-template-columns:28px minmax(0,1fr);align-items:center;gap:8px;width:100%;padding:10px 10px;border:1px solid transparent;border-radius:9px}
...
function removeRecordsRoom(){
  document.querySelectorAll('.records-wrap,.records-btn').forEach(node=>node.remove());
}
...
function boot(){
  ...
  if(target)new MutationObserver(()=>removeRecordsRoom()).observe(target,{childList:true,subtree:true});
}
