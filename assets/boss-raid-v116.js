/* Mawang Scheduler v1.2.0 - Majoku Boss Raid multi-boss catalog */
(()=>{
'use strict';
if(window.__mwsBossRaidV120)return;
window.__mwsBossRaidV120=1;

const $=id=>document.getElementById(id);
const esc=value=>String(value??'').replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
const fallbackImage=$('bossImage')?.getAttribute('src')||'';
const DEFAULT_HP=500;
let catalog={version:1,activeBossId:'boss-default',bosses:[{id:'boss-default',name:'마왕 보스',image:'',maxHealth:DEFAULT_HP,discovered:false,defeated:false}]};
...
function normalizeBoss(raw,index=0){const id=String(raw?.id||`boss-${index}`);let hp=Math.floor(Number(raw?.maxHealth));if(!Number.isFinite(hp)||hp<1)hp=DEFAULT_HP;if(id==='boss-default'&&hp===100)hp=DEFAULT_HP;return{id,name:String(raw?.name||`보스 ${index+1}`),image:String(raw?.image||''),maxHealth:Math.max(1,Math.min(1000000,hp)),discovered:Boolean(raw?.discovered),defeated:Boolean(raw?.defeated)}}
function normalizeCatalog(raw){let bosses=(Array.isArray(raw?.bosses)?raw.bosses:[]).map(normalizeBoss);if(!bosses.length)bosses=[normalizeBoss({id:'boss-default',name:'마왕 보스',maxHealth:DEFAULT_HP})];let active=String(raw?.activeBossId||'');if(!bosses.some(b=>b.id===active))active=bosses[0].id;return{version:1,activeBossId:active,bosses}}
...
function frameVisible(){try{return !window.frameElement||window.frameElement.offsetParent!==null}catch(_){return true}}
...
async function loadCatalog(force=false){
  ...
  try{const local=JSON.parse(localStorage.getItem('mwsBossRaidCatalogV116')||'null');if(local){catalog=normalizeCatalog(local);activeBossId=catalog.activeBossId;lastCatalogSignature=storeSignature(catalog)}}catch(_){}
...
async function saveBossHp(value){
  const boss=currentBoss();if(!boss)return;
  const hp=Math.max(1,Math.min(1000000,Math.floor(Number(value)||boss.maxHealth||DEFAULT_HP)));
  boss.maxHealth=hp;
  const store=hostStore();
  ...
  try{if(typeof applyBossSettings==='function')applyBossSettings(hp);else{bossState.maxHealth=hp;bossState.health=hp;bossState.running=false;bossState.finished=false;bossState.lastAttacker='';bossState.damage={};renderBossState()}}catch(error){console.warn('Boss HP apply failed',error)}
  renderBossSelect();renderCodex();
}
function bossGameActive(){return $('bossGame')?.classList.contains('active')===true}

function installStyle(){
  if($('mwsBossRaidStyleV120'))return;
  const style=document.createElement('style');style.id='mwsBossRaidStyleV120';style.textContent=`
...
function renderCodex(){const grid=$('mwsBossCodexGridV116');if(!grid)return;const known=catalog.bosses.filter(b=>b.discovered||b.defeated).length,count=$('mwsBossCodexCountV116');if(count)count.textContent=`수집 ${known} / ${catalog.bosses.length}`;grid.innerHTML=catalog.bosses.map((boss,index)=>{const knownBoss=boss.discovered||boss.defeated,image=boss.image||((boss.id==='boss-default'||index===0)?fallbackImage:'');return knownBoss?`<div class="mws-boss-codex-card-v116 known"><div class="mws-boss-codex-art-v116">${image?`<img src="${esc(image)}" alt="${esc(boss.name)}">`:'<div class="mws-boss-codex-question-v116">?</div>'}</div><div class="mws-boss-codex-name-v116">${esc(boss.name)}</div><div class="mws-boss-codex-meta-v116">HP ${boss.maxHealth.toLocaleString()}</div><span class="mws-boss-codex-badge-v116${boss.defeated?' defeated':''}">${boss.defeated?'격파 완료':'발견됨'}</span></div>`:`<div class="mws-boss-codex-card-v116"><div class="mws-boss-codex-art-v116"><div class="mws-boss-codex-question-v116">?</div></div><div class="mws-boss-codex-name-v116">???</div><div class="mws-boss-codex-meta-v116">아직 발견하지 못한 보스</div></div>`}).join('')}
function renderBossSelect(){const select=$('bossSelectV116'),input=$('bossHpInputV116'),boss=currentBoss();if(select){select.innerHTML=catalog.bosses.map(b=>`<option value="${esc(b.id)}"${b.id===activeBossId?' selected':''}>${esc(b.name)}</option>`).join('');select.disabled=safeRunning()}if(input&&boss&&!input.matches(':focus')){input.value=String(boss.maxHealth);input.disabled=safeRunning()}const apply=$('bossHpApplyV116');if(apply)apply.disabled=safeRunning();const name=$('bossStageNameV116');if(name&&boss)name.textContent=boss.name}
function safeRunning(){try{return Boolean(bossState?.running)}catch(_){return false}}
function applyCurrentBoss(discover=false){if(applying)return;const boss=currentBoss();if(!boss)return;applying=true;try{const image=boss.image||((boss.id==='boss-default'||catalog.bosses.indexOf(boss)===0)?fallbackImage:'');if($('bossImage')){$('bossImage').src=image||fallbackImage;$('bossImage').alt=boss.name}const label=$('bossStageNameV116');if(label)label.textContent=boss.name;if(!safeRunning()){try{bossState.maxHealth=boss.maxHealth;bossState.health=boss.maxHealth;bossState.finished=false;bossState.lastAttacker='';bossState.damage={};renderBossState()}catch(error){console.warn('Boss state apply failed',error)}}renderBossSelect();if(discover)markBoss({discovered:true})}finally{applying=false}}

function syncRunningState(){if(!bossGameActive()||!frameVisible())return;ensureUi();const boss=currentBoss();if(!boss)return;renderBossSelect();if(!boss.discovered)markBoss({discovered:true});try{if(bossState.finished&&bossState.health<=0&&lastFinishedBossId!==boss.id){lastFinishedBossId=boss.id;markBoss({defeated:true})}if(!bossState.finished&&lastFinishedBossId===boss.id)lastFinishedBossId=''}catch(_){}syncControllerHook()}
function syncControllerHook(){try{if(typeof bossControllerWindow==='undefined'||!bossControllerWindow||bossControllerWindow.closed)return;const doc=bossControllerWindow.document;if(!doc)return;const detail=doc.querySelector('.detail');if(detail&&detail.textContent.includes('보스 이미지는'))detail.textContent='현재 선택한 보스의 체력을 수정합니다. 적용하면 레이드 진행 상태와 딜량이 초기화됩니다.';const apply=doc.getElementById('bossControllerApply');if(apply&&!apply.dataset.v120){apply.dataset.v120='1';apply.addEventListener('click',()=>setTimeout(()=>{try{saveBossHp(bossState.maxHealth)}catch(_){}},0))}}catch(_){}}
function bindRaidButtons(){const start=$('bossStartBtn');if(start&&!start.dataset.v120){start.dataset.v120='1';start.addEventListener('click',()=>{const boss=currentBoss();if(!boss)return;try{bossState.maxHealth=boss.maxHealth;bossState.health=boss.maxHealth}catch(_){}markBoss({discovered:true})},true)}const reset=$('bossResetBtn');if(reset&&!reset.dataset.v120){reset.dataset.v120='1';reset.addEventListener('click',()=>setTimeout(()=>applyCurrentBoss(false),0))}}
function bindSectionObserver(){const boss=$('bossGame');if(!boss||boss.dataset.v120Observed)return;boss.dataset.v120Observed='1';new MutationObserver(()=>{if(bossGameActive()){applyCurrentBoss(true);renderCodex();syncRunningState()}}).observe(boss,{attributes:true,attributeFilter:['class']})}
...
async function boot(){installStyle();ensureUi();bindRaidButtons();bindSectionObserver();bindParentEvents();await loadCatalog(true);if(bossGameActive())applyCurrentBoss(true);stateTimer=setInterval(syncRunningState,1000)}
