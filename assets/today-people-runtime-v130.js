/* Mawang Scheduler v1.3.0 - Today People runtime */
(()=>{
  'use strict';
  if(window.__mwsTodayPeopleRuntimeV130)return;
  window.__mwsTodayPeopleRuntimeV130=true;
  ...
  function ensureTodayPeopleDataV130(){
  ...
  function autoIdsForDate(date){
    if(!ensureTodayPeopleDataV130()||!validDateKey(date))return [];
    const ids=new Set();
  ...
  function manualIdsForDate(date){
    if(!ensureTodayPeopleDataV130())return [];
  ...
  let lastTodayPeopleSyncSignatureV130='';
  function todayPeopleSyncSignatureV130(){
    if(!ensureTodayPeopleDataV130())return '';
    try{
  ...
  function syncTodayPeopleIfNeededV130(force=false){
    const signature=todayPeopleSyncSignatureV130();
    if(!force&&signature&&signature===lastTodayPeopleSyncSignatureV130)return false;
    syncAll();
    lastTodayPeopleSyncSignatureV130=todayPeopleSyncSignatureV130();
    return true;
  }
  window.mwsTodayPeopleSyncV130=()=>syncTodayPeopleIfNeededV130(true);
  window.mwsTodayPeopleAutoIdsV130=autoIdsForDate;
  ...
  function markAutoPickerRows(){
    ...
    try{
      ...
      document.querySelectorAll('#todayPeoplePickerList .today-people-picker-row-local[data-id]').forEach(row=>{
        ...
        if(row.hasAttribute('data-auto-v130')!==isAuto)row.toggleAttribute('data-auto-v130',isAuto);
        if(isAuto){
  ...
  function confirmTodayPeoplePickerV130(){
    if(!pickerState)return;
    const date=pickerState.date;
    ensureTodayPeopleDataV130();
    ...
    }catch(e){console.error('Mawang Scheduler v1.3.0 Today People save failed',e)}
  ...
  function preparePicker(date){
    ...
    if(list&&!list.dataset.todayPeopleV130Bound){
      list.dataset.todayPeopleV130Bound='1';
    ...
    if(confirm)confirm.onclick=confirmTodayPeoplePickerV130;
    if(modal)modal.dataset.todayPeopleV130Date=normalized;
    updateCopy();
  ...
  function install(){
    ...
    syncTodayPeopleIfNeededV130(true);

    const baseSave=window.saveData;
    const wrappedSave=function(){syncTodayPeopleIfNeededV130();return baseSave.apply(this,arguments)};
    window.saveData=wrappedSave;
    try{saveData=wrappedSave}catch(_){}

    const baseRenderAll=window.renderAll;
    const wrappedRenderAll=function(){syncTodayPeopleIfNeededV130();const out=baseRenderAll.apply(this,arguments);queueMicrotask(updateCopy);return out};
    ...
    try{if(typeof renderAll==='function')renderAll('Mawang Scheduler v1.3.0 자동 함께한 사람 동기화')}catch(e){console.error('Mawang Scheduler v1.3.0 initial render',e)}
    updateCopy();
    window.dispatchEvent(new CustomEvent('mws:today-people-runtime-ready'));
  }

  let todayPeopleInstallRetryV130=0;
  function tryInstallTodayPeopleV130(){
    if(installed)return true;
    try{install()}catch(e){console.error('Mawang Scheduler v1.3.0 install failed',e)}
    if(installed)return true;
    const delays=[120,600,1800];
    if(todayPeopleInstallRetryV130<delays.length){
      const delay=delays[todayPeopleInstallRetryV130++];
      setTimeout(tryInstallTodayPeopleV130,delay);
    }
    return false;
  }
  window.addEventListener('mws:post-login-ui-ready',tryInstallTodayPeopleV130,{once:true});
  tryInstallTodayPeopleV130();
})();
