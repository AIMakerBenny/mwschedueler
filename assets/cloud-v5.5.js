...
(()=>{
  ...
  function installFetchMeter(){
    ...
    window.fetch=async function(input,init){
      ...
      const isSb=url.startsWith(SB_URL);
      if(isSb&&init?.body)metrics.actualSupabaseRequestBytes+=byteLen(init.body);
      const res=await base(input,init);
      if(isSb){
        try{
          const copy=res.clone();
          copy.arrayBuffer().then(buf=>{metrics.actualSupabaseResponseBytes+=buf.byteLength;metric('http',{url,status:res.status,responseBytes:buf.byteLength})}).catch(()=>{});
        }catch(_){ }
      }
      return res;
    };
  ...
  async function idbTx(store,modeName,fn){
    ...
    return await new Promise((resolve,reject)=>{
      const tx=db.transaction(store,modeName),os=tx.objectStore(store);
      let result;
      try{result=fn(os)}catch(e){reject(e);return}
      tx.oncomplete=()=>resolve(result?.result);
      tx.onerror=()=>reject(tx.error||new Error('IndexedDB transaction 실패'));
      tx.onabort=()=>reject(tx.error||new Error('IndexedDB transaction 취소'));
    });
  }
  async function getCachedPart(scope,part){
    if(!CACHE_STORES.includes(part))return null;
    try{return await idbTx(part,'readonly',os=>os.get(scope))||null}catch(e){console.warn('IndexedDB cache read',part,e);return null}
  }
  async function putCachedPart(scope,part,version,value){
    if(!CACHE_STORES.includes(part))return;
    try{await idbTx(part,'readwrite',os=>os.put({scope,version:Number(version)||0,data:clone(value),cachedAt:Date.now()}))}catch(e){console.warn('IndexedDB cache write',part,e)}
  }
  async function clearCache(){
    try{const db=await openCache();await Promise.all(CACHE_STORES.map(store=>new Promise((resolve,reject)=>{const tx=db.transaction(store,'readwrite');tx.objectStore(store).clear();tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error)})))}catch(e){console.warn('IndexedDB cache clear',e)}
  }
  async function validateCacheSchema(){
    try{
      ...
      if(Number(current)!==CACHE_SCHEMA_VERSION){
        await clearCache();
        await idbTx('meta','readwrite',os=>os.put({key:'schemaVersion',value:CACHE_SCHEMA_VERSION,updatedAt:Date.now()}));
      }
  ...
  function selectLoginMode(next){
    ...
    if($('mwsLoginForm'))$('mwsLoginForm').dataset.mode=next;setLoginError('');
  }
  ...
  function applyModeUi(){
    ...
    document.querySelectorAll('.nav button').forEach(btn=>delete btn.dataset.mwsPublicHidden);applyPublicRestrictions();
    try{updateStorageStatus(true)}catch(_){}
  }

  function stripPostSources(posts){
    return (Array.isArray(posts)?posts:[]).map(p=>{const x=clone(p);for(const k of ['sourceContent','sourcePhotos','sourceAuthor','sourceAuthorId','sourceRegDate','sourceViewCount','sourceUrl'])delete x[k];return x});
  }
  function extractPart(part,src=data){
    ...
    if(part==='contacts')return clone(Array.isArray(d.contacts)?d.contacts:[]);
    if(part==='events')return clone(Array.isArray(d.events)?d.events:[]);
    if(part==='posts')return stripPostSources(d.posts);
    ...
    if(part==='clipboard')return {scheduleClipboard:clone(Array.isArray(d.scheduleClipboard)?d.scheduleClipboard:[])};
    return null;
  }
  function mergePart(part,value,target=data){
    const v=clone(value);
    if(part==='core'){Object.assign(target,v||{});return}
    if(part==='contacts'){target.contacts=Array.isArray(v)?v:[];return}
    if(part==='events'){target.events=Array.isArray(v)?v:[];return}
    if(part==='posts'){target.posts=Array.isArray(v)?v:[];return}
    if(part==='miniGames'){target.miniGames=v&&typeof v==='object'?v:{};return}
    if(['activity','notebook','contactMeta','clipboard'].includes(part))Object.assign(target,v&&typeof v==='object'?v:{});
  }
  function freshSkeleton(){
    return {version:52,categories:[],contacts:[],events:[],posts:[],miniGames:{},collaborations:[],todayPeopleByDate:{},todayPeopleManualByDate:{},targetList:[],memos:[],favoriteFolders:[],contactTags:[],contactTagBanners:{},scheduleClipboard:[],timezones:[]};
  }
  function normalizeAndRender(reason){
  ...
  async function fetchManifest(scope){
    ...
    const res=await timeout(sb.rpc('get_workspace_manifest',{p_scope:scope}),10000,'버전 확인');
    if(res.error)throw res.error;
    const m=res.data||{};const bytes=byteLen(m);metrics.semanticResponseBytes+=bytes;metric('manifest',{scope,bytes,ms:Math.round(performance.now()-started)});
    return m;
  }
  async function fetchPart(scope,part){
    const started=performance.now();
    const res=await timeout(sb.from('workspace_parts').select('part,version,data').eq('scope',scope).eq('part',part).maybeSingle(),12000,`${part} 불러오기`);
    if(res.error)throw res.error;if(!res.data)throw new Error(`${part} 데이터가 없습니다.`);
  ...
  async function loadPartWithCache(scope,part,serverManifest,{allowStale=true}={}){
    ...
    const cached=await getCachedPart(scope,part);
  ...
  async function hydrate(chosen){
    ...
    metric('hydrate-complete',{scope,offline,loaded:[...loadedParts]});
  }

  async function ensurePartsForTab(tab){
    const extras=PARTS_BY_TAB[tab]||[];if(!extras.length)return true;
    const scope=mode==='admin'?'admin':'public';
    if(!manifest){try{manifest=await fetchManifest(scope)}catch(_){} }
    let changed=false;
    for(const part of extras){
      if(loadedParts.has(part))continue;
      try{const row=await loadPartWithCache(scope,part,manifest,{allowStale:true});mergePart(part,row.data,data);loadedParts.add(part);baselineJson.set(part,JSON.stringify(row.data));changed=true}
      catch(e){console.error('Lazy part load failed',part,e);try{toast('데이터 불러오기',`${part} 데이터를 불러오지 못했습니다.`)}catch(_){} }
    }
  ...
  function markDirty(){
    ...
    const changed=[];
    for(const part of loadedParts){const current=JSON.stringify(extractPart(part));if(current!==baselineJson.get(part)){dirtyParts.add(part);changed.push(part)}else dirtyParts.delete(part)}
    return changed;
  }
  ...
  async function writeCloudNow(manual=false){
    if(mode!=='admin'||!sb)return false;if(cloudSaving){pendingSave=true;return false}markDirty();
    if(!dirtyParts.size){if(manual){try{toast('SAVE','변경된 데이터가 없습니다.')}catch(_){}}return true}
    cloudSaving=true;const partsToSave=[...dirtyParts];
    if(manual)setManualSaveProgress(8,'PREP');
    try{
  ...
  function installSaveBridge(){
    ...
    try{persist=devicePersist;window.persist=devicePersist}catch(_){}
    ...
    try{saveData=v54Save;window.saveData=v54Save}catch(_){}
  }

  async function isCurrentUserAdmin(){
    const {data:{user}}=await timeout(sb.auth.getUser(),10000,'Admin 권한 확인');if(!user)return false;
    const {data:rows,error}=await timeout(sb.from('admin_profiles').select('user_id').eq('user_id',user.id).limit(1),10000,'Admin 권한 조회');
    if(!error&&Array.isArray(rows)&&rows.length===1){await setMeta('verifiedAdminUserId',user.id);return true}return false;
  }
  async function adminLogin(username,password){
    const email=safeUserEmail(username);let res=await timeout(sb.auth.signInWithPassword({email,password}),12000,'Admin 로그인');
    if(res.error){
    ...
    if(res.error)throw res.error;if(!(await isCurrentUserAdmin())){await sb.auth.signOut();throw new Error('Admin 권한이 없는 계정입니다.')}
  }
  async function handleLogin(ev){
    ev.preventDefault();const chosen=$('mwsLoginForm')?.dataset.mode||'admin',btn=$('mwsLoginSubmit');if(btn)btn.disabled=true;setLoginError('');
    try{if(chosen==='admin'){const u=$('mwsLoginId')?.value.trim(),p=$('mwsLoginPw')?.value;if(!u||!p)throw new Error('Admin 아이디와 비밀번호를 입력해 주세요.');await adminLogin(u,p)}else await sb.auth.signOut();rememberChoice(chosen);await hydrate(chosen)}
    catch(e){setLoginError(e.message||String(e))}finally{if(btn)btn.disabled=false}
  }
  ...
  async function loadAdminList(){
    ...
    if(error){list.innerHTML=`<div class="mws-login-error">${typeof esc==='function'?esc(error.message):error.message}</div>`;return}
    list.innerHTML=(rows||[]).map((r,i)=>`<div class="mws-admin-row"><div><div style="font-weight:850">${typeof esc==='function'?esc(r.username):r.username}</div><small>${i===0?'초기 Admin':'추가 Admin'}</small></div><small>${new Date(r.created_at).toLocaleDateString('ko-KR')}</small></div>`).join('')||'<div class="subtle">Admin 계정이 없습니다.</div>';
  }
  async function addAdmin(ev){
    ev.preventDefault();const msg=$('mwsAdminManageMsg');if(msg)msg.textContent='';const username=$('mwsNewAdminId')?.value.trim(),password=$('mwsNewAdminPw')?.value;
    try{const {data:result,error}=await sb.functions.invoke('create-admin',{body:{username,password}});if(error||result?.error)throw new Error(result?.error||error?.message||'추가 실패');$('mwsNewAdminId').value='';$('mwsNewAdminPw').value='';if(msg){msg.style.color='var(--ok)';msg.textContent=`${result.username} Admin 계정을 추가했습니다.`}await loadAdminList()}
    catch(e){if(msg){msg.style.color='var(--danger)';msg.textContent=e.message||String(e)}}
  }

  function setManualSaveProgress(percent,phase='SAVE'){
    const btn=$('mwsSidebarSaveBtn');if(!btn)return;const p=Math.max(0,Math.min(100,Math.round(Number(percent)||0)));btn.style.setProperty('--save-progress-v574',`${p}%`);
    const label=btn.querySelector('.sidebar-save-label-v572');if(label)label.textContent=p>=100?'SAVED 100%':phase==='FAILED'?'SAVE FAILED':`${phase} ${p}%`;
    if(p>=100)setTimeout(()=>{btn.style.removeProperty('--save-progress-v574');if(label)label.textContent='SAVE'},1100);
  }
  ...
  function bindUi(){
    $('mwsModeAdmin')?.addEventListener('click',()=>selectLoginMode('admin'));$('mwsModePublic')?.addEventListener('click',()=>selectLoginMode('public'));
    $('mwsLoginForm')?.addEventListener('submit',handleLogin);$('mwsSidebarSaveBtn')?.addEventListener('click',manualSave);
  ...
  async function canUseOfflineAdmin(session){
    if(!session?.user)return false;const verified=await getMeta('verifiedAdminUserId');if(verified?.value!==session.user.id)return false;
    for(const part of INITIAL_PARTS)if(!await getCachedPart('admin',part))return false;return true;
  }
  ...
  async function init(){
    ...
    try{
      ...
      if(remember&&preferred==='admin'){
        const {data:{session}}=await timeout(sb.auth.getSession(),8000,'Admin 세션 확인');
        if(session){
          try{if(await isCurrentUserAdmin()){await hydrate('admin');installLazyTabBridge();return}}catch(e){if(await canUseOfflineAdmin(session)){await hydrate('admin');installLazyTabBridge();return}throw e}
        }
      }
