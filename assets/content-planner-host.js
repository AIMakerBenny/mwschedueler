/* CF MWS V 1.0.5 - Content Planner category integration + scale-safe UI */
(()=>{
  'use strict';
  if(window.__mwsContentPlannerHostV105)return;
  window.__mwsContentPlannerHostV105=true;
  ...
  function ensureStyle(){
    if(document.getElementById('mwsContentPlannerHostStyleV105'))return;
    const style=document.createElement('style');
    style.id='mwsContentPlannerHostStyleV105';
    style.textContent=`
      ...
      #mwsContentPlannerFrame{display:block;width:100%;height:100%;border:0;background:#fff}
  ...
  function install(){forceVersionV105();ensureStyle();ensureSection();ensureNav()}
  ...
  window.addEventListener('mawang:datachange',()=>setTimeout(()=>{forceVersionV105();sendContacts()},0));

  install();[100,300,800,1600,3000].forEach(ms=>setTimeout(install,ms));
  setTimeout(()=>{forceVersionV105();const body=document.body;const label=document.querySelector('[id^="mwsBuildVersionV5"],#mwsBuildVersion,[class*="sidebar-build-version"]');if(body&&!body.__mwsV105BuildObserver){const bo=new MutationObserver(()=>{if(body.getAttribute('data-build-version')!==BUILD)forceVersionV105()});bo.observe(body,{attributes:true,attributeFilter:['data-build-version']});body.__mwsV105BuildObserver=bo}if(label&&!label.__mwsV105BuildObserver){const o=new MutationObserver(()=>{if(label.textContent!==BUILD)forceVersionV105()});o.observe(label,{childList:true,characterData:true,subtree:true});label.__mwsV105BuildObserver=o}},3200);
})();
