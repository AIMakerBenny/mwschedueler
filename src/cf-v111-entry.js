...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/assets/cloud-v5.5.js') {
      const replacement = new URL('/assets/cloud-v1.1-loader.js?v=1.1.5', request.url);
    ...
    if (url.pathname === '/majoku-castle.html') {
      ...
      return htmlResponse(response,html,{'x-mws-majoku-sidebar':'v1.1.5'});
    }
    ...
    } else if(!html.includes('ui-fixes-v113.js')) {
      html = html.includes('</body>') ? html.replace('</body>', `${UI_FIX_SCRIPT}\n</body>`) : `${html}\n${UI_FIX_SCRIPT}`;
    }
