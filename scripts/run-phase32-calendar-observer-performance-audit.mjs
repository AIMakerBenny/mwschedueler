...
export function runPhase32CalendarObserverPerformanceAudit(){
  ...
  if(!/(?:observe\(document\.getElementById\('calendarGrid'\),\{childList:true\}\)|observe\(calendarGrid,\{childList:true\}\))/.test(src))issues.push('device refresh no longer observes direct calendar rebuilds');
  if(!src.includes("observe(grid,{childList:true})"))issues.push('mobile calendar view no longer observes direct calendar rebuilds');
  if(!src.includes('if(document.hidden||queued)return')&&!(src.includes('if(document.hidden||calendarQueued)return')&&src.includes('if(document.hidden||fullQueued)return')))issues.push('device UI refresh is not suspended in hidden tabs');
